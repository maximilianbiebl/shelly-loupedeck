...
namespace ShellyLoupedeckPlugin.Actions
{
    public class CustomFolder8 : PluginDynamicFolder
    {
        ...
        public override void RunCommand(string actionParameter)
        {
            ...
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder8] Navigate Up pressed");
                ...
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder8] After Navigate Up, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder8] Calling ButtonActionNamesChanged() after navigate up");
