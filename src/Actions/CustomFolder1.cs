...
{
    public class CustomFolder1 : PluginDynamicFolder
    {
        ...
        public override bool Activate()
        {
            ...
            DebugLogger.Log("[CustomFolder1] Folder activated, resetting to main menu");
            ...
            _refreshTimer = new System.Threading.Timer(
                _ => {
                    ...
                    {
                        DebugLogger.Log($"[CustomFolder1] Timer refresh failed: {ex.Message}");
                    }
                },
                null,
        ...
        private void OnDevicesUpdated(object sender, EventArgs e)
        {
            ...
            {
                DebugLogger.Log($"[CustomFolder1] OnDevicesUpdated ButtonActionNamesChanged failed: {ex.Message}");
            }
        }
        ...
        private void OnFoldersUpdated(object sender, EventArgs e)
        {
            ...
            {
                DebugLogger.Log($"[CustomFolder1] OnFoldersUpdated ButtonActionNamesChanged failed: {ex.Message}");
            }
        }
        ...
        public override IEnumerable<string> GetButtonPressActionNames()
        {
            ...
            if (!string.IsNullOrEmpty(_currentSubmenu))
            {
                ...
                DebugLogger.Log($"[CustomFolder1] Invalid submenu: {_currentSubmenu}, resetting to main menu");
                _currentSubmenu = null;
        ...
        {
            DebugLogger.Log($"[CustomFolder1] RunCommand called: {actionParameter}");
            DebugLogger.Log($"[CustomFolder1] Current submenu: {_currentSubmenu ?? "null"}");

            // SDK Navigate Up - only exits folder (used on main menu)
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder1] SDK Navigate Up pressed (exit folder)");
                return;
            }

            var cmdParts = actionParameter.Split('_');
            DebugLogger.Log($"[CustomFolder1] Command parts: {string.Join(", ", cmdParts)}");

            // Custom back button - goes up one level
            if (cmdParts[0] == "back")
            {
                DebugLogger.Log("[CustomFolder1] Custom Back pressed");
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder1] After Back, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder1] Calling ButtonActionNamesChanged() after back");
                        ButtonActionNamesChanged();
                    }
                    catch (Exception ex)
                    {
            ...
            if (cmdParts[0] == "opendevice" && cmdParts.Length >= 2)
            {
                DebugLogger.Log($"[CustomFolder1] Opening device settings for: {cmdParts[1]}");
            ...
            if (cmdParts[0] == "openaction" && cmdParts.Length >= 3)
            {
                var actionType = cmdParts[1]; // brightness, dim, color, temperature
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder1] Opening action submenu {actionType} for: {deviceId}");
            ...
            if (cmdParts[0] == "devicesetting" && cmdParts.Length >= 3)
            {
                var settingType = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder1] Opening {settingType} menu for: {deviceId}");
            ...
            if (cmdParts[0] == "colorpreset" && cmdParts.Length >= 3)
            {
                var preset = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder1] Applying color preset {preset} to: {deviceId}");
            ...
            if (cmdParts[0] == "colormenu" && cmdParts.Length >= 3)
            {
                var channel = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder1] Opening color {channel} menu for: {deviceId}");
