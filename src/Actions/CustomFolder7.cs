...
{
    public class CustomFolder7 : PluginDynamicFolder
    {
        ...
        public CustomFolder7()
        {
            DisplayName = "Custom Folder 7";
        ...
        public override bool Activate()
        {
            ...
            DebugLogger.Log("[CustomFolder7] Folder activated, resetting to main menu");
            ...
            _refreshTimer = new System.Threading.Timer(
                _ => {
                    ...
                    {
                        DebugLogger.Log($"[CustomFolder7] Timer refresh failed: {ex.Message}");
                    }
                },
                null,
        ...
        private void OnDevicesUpdated(object sender, EventArgs e)
        {
            ...
            {
                DebugLogger.Log($"[CustomFolder7] OnDevicesUpdated ButtonActionNamesChanged failed: {ex.Message}");
            }
        }
        ...
        private void OnFoldersUpdated(object sender, EventArgs e)
        {
            ...
            {
                DebugLogger.Log($"[CustomFolder7] OnFoldersUpdated ButtonActionNamesChanged failed: {ex.Message}");
            }
        }
        ...
        private void UpdateDisplayName()
        {
            ...
            if (folder != null)
                ...
                DisplayName = "Custom Folder 7 (Empty)";
        }
        ...
        public override IEnumerable<string> GetButtonPressActionNames()
        {
            ...
            if (!string.IsNullOrEmpty(_currentSubmenu))
            {
                ...
                DebugLogger.Log($"[CustomFolder7] Invalid submenu: {_currentSubmenu}, resetting to main menu");
                _currentSubmenu = null;
        ...
        {
            DebugLogger.Log($"[CustomFolder7] RunCommand called: {actionParameter}");
            DebugLogger.Log($"[CustomFolder7] Current submenu: {_currentSubmenu ?? "null"}");

            // SDK Navigate Up - only exits folder (used on main menu)
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder7] SDK Navigate Up pressed (exit folder)");
                return;
            }

            var cmdParts = actionParameter.Split('_');
            DebugLogger.Log($"[CustomFolder7] Command parts: {string.Join(", ", cmdParts)}");

            // Custom back button - goes up one level
            if (cmdParts[0] == "back")
            {
                DebugLogger.Log("[CustomFolder7] Custom Back pressed");
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder7] After Back, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder7] Calling ButtonActionNamesChanged() after back");
                        ButtonActionNamesChanged();
                    }
                    catch (Exception ex)
                    {
                        DebugLogger.Log($"[CustomFolder7] ButtonActionNamesChanged failed: {ex.Message}");
                    }
                }
                return;
            }
            ...
            if (cmdParts[0] == "opendevice" && cmdParts.Length >= 2)
            {
                DebugLogger.Log($"[CustomFolder7] Opening device settings for: {cmdParts[1]}");
                _currentSubmenu = $"device_{cmdParts[1]}";
                DebugLogger.Log($"[CustomFolder7] Set submenu to: {_currentSubmenu}");

                // Force folder refresh by notifying button list changed
                try
                {
                    DebugLogger.Log($"[CustomFolder7] Calling ButtonActionNamesChanged()");
                    ButtonActionNamesChanged();
                }
                catch (Exception ex)
                {
            ...
            if (cmdParts[0] == "openaction" && cmdParts.Length >= 3)
            {
                var actionType = cmdParts[1]; // brightness, dim, color, temperature
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder7] Opening action submenu {actionType} for: {deviceId}");
            ...
            if (cmdParts[0] == "devicesetting" && cmdParts.Length >= 3)
            {
                var settingType = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder7] Opening {settingType} menu for: {deviceId}");
            ...
            if (cmdParts[0] == "colorpreset" && cmdParts.Length >= 3)
            {
                var preset = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder7] Applying color preset {preset} to: {deviceId}");
            ...
            {
                var groupId = cmdParts[1];
                DebugLogger.Log($"[CustomFolder7] Opening group color menu for: {groupId}");
            ...
            if (cmdParts[0] == "groupcolorpreset" && cmdParts.Length >= 3)
            {
                ...
                DebugLogger.Log($"[CustomFolder7] Applying group color preset {preset} to: {groupId}");
            ...
            if (cmdParts[0] == "groupcolormenu" && cmdParts.Length >= 3)
            {
                var channel = cmdParts[1];
                var groupId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder7] Opening group color {channel} menu for: {groupId}");
            ...
            if (cmdParts[0] == "colormenu" && cmdParts.Length >= 3)
            {
                var channel = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder7] Opening color {channel} menu for: {deviceId}");
        ...
        private async System.Threading.Tasks.Task AdjustDeviceBrightnessAsync(string deviceId, int adjustment)
        {
            ...
            if (device == null)
            {
                DebugLogger.Log($"[CustomFolder7] AdjustDeviceBrightnessAsync: Device {deviceId} not found");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder7] Adjusting brightness for {deviceId}: {currentBrightness} -> {newBrightness}");

            bool success = await _plugin.ApiClient.SetLightBrightnessAsync(deviceId, newBrightness);

            if (!success)
            {
                DebugLogger.Log($"[CustomFolder7] SetLightBrightnessAsync failed for {deviceId}");
                return;
            }
        ...
        private async System.Threading.Tasks.Task AdjustDeviceTemperatureAsync(string deviceId, double adjustment)
        {
            ...
            if (device == null)
            {
                DebugLogger.Log($"[CustomFolder7] AdjustDeviceTemperatureAsync: Device {deviceId} not found");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder7] Adjusting temperature for {deviceId}: {currentTemp} -> {newTemp}");

            bool success = await _plugin.ApiClient.SetThermostatTemperatureAsync(deviceId, newTemp);

            if (!success)
            {
                DebugLogger.Log($"[CustomFolder7] SetThermostatTemperatureAsync failed for {deviceId}");
                return;
            }
        ...
        private async System.Threading.Tasks.Task AdjustDeviceColorChannelAsync(string deviceId, string channel, int adjustment)
        {
            ...
            if (device == null)
            {
                DebugLogger.Log($"[CustomFolder7] AdjustDeviceColorChannelAsync: Device {deviceId} not found");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder7] Adjusting color {channel} for {deviceId}: RGB({r},{g},{b})");
            ...
            if (!success)
            {
                DebugLogger.Log($"[CustomFolder7] SetLightColorAsync failed for {deviceId}");
                return;
            }
        ...
        private async System.Threading.Tasks.Task AdjustGroupColorChannelAsync(string groupId, string channel, int adjustment)
        {
            ...
            if (group == null)
            {
                DebugLogger.Log($"[CustomFolder7] AdjustGroupColorChannelAsync: Group {groupId} not found");
                return;
            }
            ...
            if (firstDevice == null)
            {
                DebugLogger.Log($"[CustomFolder7] AdjustGroupColorChannelAsync: No devices found in group {groupId}");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder7] Adjusting group color {channel} for group {groupId}: RGB({r},{g},{b})");
