...
namespace ShellyLoupedeckPlugin.Actions
{
    public class CustomFolder7 : PluginDynamicFolder
    {
        ...
        public override void RunCommand(string actionParameter)
        {
            ...
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder7] Navigate Up pressed");
                ...
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder7] After Navigate Up, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder7] Calling ButtonActionNamesChanged() after navigate up");
