...
namespace ShellyLoupedeckPlugin.Actions
{
    public class CustomFolder6 : PluginDynamicFolder
    {
        ...
        public override void RunCommand(string actionParameter)
        {
            ...
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder6] Navigate Up pressed");
                ...
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder6] After Navigate Up, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder6] Calling ButtonActionNamesChanged() after navigate up");
