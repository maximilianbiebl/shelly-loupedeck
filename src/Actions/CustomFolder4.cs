...
{
    public class CustomFolder4 : PluginDynamicFolder
    {
        ...
        public CustomFolder4()
        {
            DisplayName = "Custom Folder 4";
        ...
        private void OnDevicesUpdated(object sender, EventArgs e)
        {
            ...
            {
                DebugLogger.Log($"[CustomFolder4] OnDevicesUpdated ButtonActionNamesChanged failed: {ex.Message}");
            }
        }
        ...
        private void OnFoldersUpdated(object sender, EventArgs e)
        {
            ...
            {
                DebugLogger.Log($"[CustomFolder4] OnFoldersUpdated ButtonActionNamesChanged failed: {ex.Message}");
            }
        }
        ...
        private void UpdateDisplayName()
        {
            ...
            if (folder != null)
                ...
                DisplayName = "Custom Folder 4 (Empty)";
        }
        ...
        public override IEnumerable<string> GetButtonPressActionNames()
        {
            ...
            if (!string.IsNullOrEmpty(_currentSubmenu))
            {
                ...
                DebugLogger.Log($"[CustomFolder4] Invalid submenu: {_currentSubmenu}, resetting to main menu");
                _currentSubmenu = null;
        ...
        public override void RunCommand(string actionParameter)
        {
            ...
            if (cmdParts[0] == "colorpreset" && cmdParts.Length >= 3)
            {
                var preset = cmdParts[1];
                var deviceId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder4] Applying color preset {preset} to: {deviceId}");
            ...
            {
                var groupId = cmdParts[1];
                DebugLogger.Log($"[CustomFolder4] Opening group color menu for: {groupId}");
            ...
            if (cmdParts[0] == "groupcolorpreset" && cmdParts.Length >= 3)
            {
                ...
                DebugLogger.Log($"[CustomFolder4] Applying group color preset {preset} to: {groupId}");
            ...
            if (cmdParts[0] == "groupcolormenu" && cmdParts.Length >= 3)
            {
                var channel = cmdParts[1];
                var groupId = cmdParts[2];
                DebugLogger.Log($"[CustomFolder4] Opening group color {channel} menu for: {groupId}");
        ...
        private async System.Threading.Tasks.Task AdjustDeviceBrightnessAsync(string deviceId, int adjustment)
        {
            ...
            if (device == null)
            {
                DebugLogger.Log($"[CustomFolder4] AdjustDeviceBrightnessAsync: Device {deviceId} not found");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder4] Adjusting brightness for {deviceId}: {currentBrightness} -> {newBrightness}");

            bool success = await _plugin.ApiClient.SetLightBrightnessAsync(deviceId, newBrightness);

            if (!success)
            {
                DebugLogger.Log($"[CustomFolder4] SetLightBrightnessAsync failed for {deviceId}");
                return;
            }
        ...
        private async System.Threading.Tasks.Task AdjustDeviceTemperatureAsync(string deviceId, double adjustment)
        {
            ...
            if (device == null)
            {
                DebugLogger.Log($"[CustomFolder4] AdjustDeviceTemperatureAsync: Device {deviceId} not found");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder4] Adjusting temperature for {deviceId}: {currentTemp} -> {newTemp}");

            bool success = await _plugin.ApiClient.SetThermostatTemperatureAsync(deviceId, newTemp);

            if (!success)
            {
                DebugLogger.Log($"[CustomFolder4] SetThermostatTemperatureAsync failed for {deviceId}");
                return;
            }
        ...
        private async System.Threading.Tasks.Task AdjustDeviceColorChannelAsync(string deviceId, string channel, int adjustment)
        {
            ...
            if (device == null)
            {
                DebugLogger.Log($"[CustomFolder4] AdjustDeviceColorChannelAsync: Device {deviceId} not found");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder4] Adjusting color {channel} for {deviceId}: RGB({r},{g},{b})");
            ...
            if (!success)
            {
                DebugLogger.Log($"[CustomFolder4] SetLightColorAsync failed for {deviceId}");
                return;
            }
        ...
        private async System.Threading.Tasks.Task AdjustGroupColorChannelAsync(string groupId, string channel, int adjustment)
        {
            ...
            if (firstDevice == null)
            {
                DebugLogger.Log($"[CustomFolder4] AdjustGroupColorChannelAsync: No devices found in group {groupId}");
                return;
            }
            ...
            DebugLogger.Log($"[CustomFolder4] Adjusting group color {channel} for group {groupId}: RGB({r},{g},{b})");
