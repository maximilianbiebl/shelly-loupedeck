...
namespace ShellyLoupedeckPlugin.Actions
{
    public class CustomFolder3 : PluginDynamicFolder
    {
        ...
        public override void RunCommand(string actionParameter)
        {
            ...
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder3] Navigate Up pressed");
                ...
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder3] After Navigate Up, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder3] Calling ButtonActionNamesChanged() after navigate up");
