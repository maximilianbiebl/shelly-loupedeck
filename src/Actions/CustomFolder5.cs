...
namespace ShellyLoupedeckPlugin.Actions
{
    public class CustomFolder5 : PluginDynamicFolder
    {
        ...
        public override void RunCommand(string actionParameter)
        {
            ...
            if (actionParameter == PluginDynamicFolder.NavigateUpActionName)
            {
                DebugLogger.Log("[CustomFolder5] Navigate Up pressed");
                ...
                if (!string.IsNullOrEmpty(_currentSubmenu))
                {
                    ...
                    DebugLogger.Log($"[CustomFolder5] After Navigate Up, submenu: {_currentSubmenu ?? "null"}");

                    // Force folder refresh
                    try
                    {
                        DebugLogger.Log($"[CustomFolder5] Calling ButtonActionNamesChanged() after navigate up");
